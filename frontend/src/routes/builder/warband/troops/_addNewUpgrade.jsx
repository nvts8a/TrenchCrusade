...
export default function AddNewUpgrade({warband, troop, troopType, currentUpgrades}) {
    ...
    const upgradeCards = troopType.troopTypeUpgrades
            ...
            .map((troopTypeUpgrade) => {
                const currentlyUpgraded = currentUpgrades ? currentUpgrades.filter((currentUpgrade) => currentUpgrade.upgrade.id === troopTypeUpgrade.upgrade.id).length > 0 : []
                ...
                const upgradeModifiers = () => {
                    if (troopTypeUpgrade.upgrade.upgradeModifiers.length > 0) {
                        const modifiersAsRules = troopTypeUpgrade.upgrade.upgradeModifiers.map((modifier) => {
                            const key = `${modifier.type}-${modifier.value}`
                            switch (modifier.type) {
                                case 'keyword':
                                    return({id: key, name: loader.keywords[modifier.value].name, rule: loader.keywords[modifier.value].definition})
                                case 'equipment':
                                    return({id: key, name: 'Equipment', rule: loader.equipment[modifier.value].name})
                                case 'armor':
                                    return({id: key, name: 'Armour', rule: modifier.value})
                                case 'melee':
                                    return({id: key, name: 'Melee',  rule: (modifier.value < 0) ? `${modifier.value} Dice` : `+${modifier.value} Dice` })
                                case 'range':
                                    return({id: key, name: 'Ranged', rule: (modifier.value < 0) ? `${modifier.value} Dice` : `+${modifier.value} Dice` })
                                default:
                                    return({name: modifier.type, rule: modifier.value })
                            }
                        })
                        return(<Rules key='upgrade-modifiers' rules={modifiersAsRules} />)
                    }
                }
                ...
                const upgradeRules = () => {
                    if (troopTypeUpgrade.upgrade.upgradeRules.length > 0) {
                        ...
                        return(<Rules key='upgrade-rules' rules={upgradeRules} />)
                    }
                }

                return(
                    <Card key={troopTypeUpgrade.id}>
                        <Card.Header>
