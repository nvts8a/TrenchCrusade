    ...
    return(
        <PageLayout pageName='Charts'>
            ...
            <Chart title='Injury Chart' key='injury-chart'>
                ...
                <p>Note: On occasion, a rule will ask you to total more (or less) than 2d6 to determine the outcome of a roll. If the dice roll is modified by + DICE or - DICE when you do so, modify the dice pool as normal and keep the highest or lowest number of dice equal to number dice in the starting dice pool. For example, if the rule asked you to roll a total of 3d6 with +1 DICE, you would roll four dice and keep and total the highest three dice.</p>
                <Table bordered>
                    <tbody>
                        <tr>
                ...
                <p>A model that has suffered a Down result is in serious trouble! Add +1 BLOOD MARKER next to the model and place the model face down on the battlefield to indicate its status. If the model is already Down, it suffers +2 BLOOD MARKERS and remains Down.</p>
                ...
                <p>During its own Activations a Downed warrior can stand up, but all types of movement it takes (Move, Dash, Charge, Jump etc.) is at half distance, rounding down. A model that is Down cannot move at all during its Activation.</p>
